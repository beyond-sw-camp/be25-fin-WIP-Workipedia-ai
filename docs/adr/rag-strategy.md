...
- 후속 질문은 세션 컨텍스트를 참고해 독립된 `retrieval_query`로 재작성한다. 검색·reranking과 Tool 선택에는 `retrieval_query`를 사용하고, 최종 답변 생성에는 마스킹된 원본 질문과 세션 기록을 사용한다.
- 컨텍스트 선택·마스킹·검색어 재작성은 `ChatbotService`가 담당하고, 오케스트레이터는 이미 마스킹된 입력만 조율한다.
- contextualize 실패는 원본 질문으로 fallback하고 `CONTEXT/ERROR` 이력을 남긴다. 예상하지 못한 구현 오류는 전파한다.
...
- contextualize는 provider HTTP timeout `25초`, outer timeout `30초`를 기본값으로 사용한다.
- `custom_prompt`는 일반 사용자가 작성하지 않으며 신뢰된 BE 내부 계약에서만 전달한다.
