...
| `rag_citations` | 챗봇 RAG 답변에서 참조한 출처 인용 이력 |
...
- 성공 응답 출처는 `candidate_id`, `source_type`, `source_id`, `chunk_index`(nullable), `page_start`/`page_end`(PDF 기반 MANUAL인 경우 nullable), `title`, `score`를 포함한다.
- AI는 최종 답변에 실제 사용된 출처 식별 계약까지만 제공하고, `rag_citations` 저장·중복 제거/집계 정책·FAQ 인기 매뉴얼 캐시 무효화는 BE가 처리한다.
