...
class Settings(BaseSettings):
    ...
    # True면 '출처마다 코사인 1위 1개'를 강제로 뽑는다. 그런데 답이 한 출처(예: 회사소개
    # 매뉴얼)에 여러 청크로 흩어진 질문("한화 에어로스페이스가 뭐야?")에서는, 그 출처의 대표
    # 1개가 하필 목차/머리말 보일러플레이트로 뽑혀 알맹이 청크를 전부 버리고 NO_RESULT가 난다.
    # 또한 무관 출처도 1개씩 강제 포함돼 노이즈 카드가 생긴다. 그래서 기본값을 False로 두어
    # 상대 마진 컷(_finalize_candidates) 경로를 쓴다 — 관련 높은 출처의 여러 청크를 살리고
    # 코사인이 낮은 무관 출처는 자동 탈락시킨다.
    rag_source_balanced: bool = False
