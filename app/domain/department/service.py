...
- 대상 부서의 현재 역할이 있으면 절대 삭제하지 말고, 현재 역할에 변경 지시의 역할을 추가·병합한 최종 설명을 반환합니다.
- 변경 지시는 기존 역할을 대체하는 문장이 아니라 기존 역할에 더할 내용입니다. 명시적으로 "삭제", "제거", "교체", "대체"하라는 지시가 있을 때만 기존 역할을 줄이거나 바꿉니다.
- 문자열을 단순히 이어 붙이지 않고, 기존 역할과 추가 역할을 자연스러운 한 문장으로 병합합니다.
...
def _parse_llm_response(content: str, targets: list[DepartmentTarget], instruction: str) -> list[DepartmentRoutingResult]:
    target_map = {t.department_id: t for t in targets}
    ...
    for item in raw_results:
        ...
        merged_prompt = _preserve_current_prompt(target_map[dept_id].current_prompt, prompt, instruction)
        masked_prompt = masker.mask(merged_prompt)
        results.append(DepartmentRoutingResult(department_id=dept_id, routing_prompt=masked_prompt))
    return results


def _preserve_current_prompt(current_prompt: str, new_prompt: str, instruction: str) -> str:
    trimmed_current = current_prompt.strip()
    trimmed_new = new_prompt.strip()
    if not trimmed_current or _allows_replace(instruction) or _contains_prompt(trimmed_new, trimmed_current):
        return trimmed_new
    return f"{trimmed_current}\n{trimmed_new}"


def _allows_replace(instruction: str) -> bool:
    return any(keyword in instruction for keyword in _REPLACE_KEYWORDS)


def _contains_prompt(value: str, expected: str) -> bool:
...
class DepartmentRoutingPromptService:
    def generate(self, request: RoutingPromptRequest) -> RoutingPromptResponse:
        ...
        results = _parse_llm_response(content, request.targets, request.instruction)
