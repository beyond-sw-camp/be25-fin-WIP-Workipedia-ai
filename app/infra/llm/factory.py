...
def get_llm(request_timeout: float | None = None) -> BaseChatModel:
    ...
    if settings.llm_provider == LLMProvider.LOCAL:
        return OllamaClient().get_model(request_timeout)

    if settings.llm_provider == LLMProvider.OPENAI:
        return OpenAIClient().get_model(request_timeout)

    if settings.llm_provider == LLMProvider.GOOGLE:
        return GoogleClient().get_model(request_timeout)

    if settings.llm_provider == LLMProvider.ANTHROPIC:
        return AnthropicClient().get_model(request_timeout)

    if settings.llm_provider == LLMProvider.FALLBACK:
        candidates = []
        if settings.openai_api_key:
            candidates.append(OpenAIClient().get_model(request_timeout))
        if settings.google_api_key:
            candidates.append(GoogleClient().get_model(request_timeout))
        if settings.anthropic_api_key:
            candidates.append(AnthropicClient().get_model(request_timeout))
        if not candidates:
