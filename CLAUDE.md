...
- 구현 중 자동으로 커밋하지 않는다. 변경 사항을 staged/unstaged 상태로 두고 사용자가 직접 검토 후 커밋한다.
- 커밋 메시지는 한국어로 작성한다. 예: `feat: RAG 검색 파이프라인 구현`
...
이슈 작성 시 `.github/ISSUE_TEMPLATE/` 양식을 따른다. 종류는 세 가지다.
